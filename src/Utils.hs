...
showIncorrect incorrect =
  ...
  where
    guesses = T.intersperse ' ' (T.pack $ sort $ Set.elems incorrect)
...
winGame (Target t) correct =
  if correct == haystack
    then Win
    else Continue
  where
    haystack = Set.fromList $ T.unpack t
